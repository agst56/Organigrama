...
def retrieveOrgInit(COD_ORG):
    ...
    dependencia = Dependencia(result['cod_dep'], result['nom_dep'], responsable)
    return dependencia


def retrieveDepSuc(COD_DEP):
    columns = '''dep_suc.COD_DEP as COD_DEP, dep_suc.NOM as NOM_DEP, p.COD_PER, 
    p.NOM as NOM_PER, p.APE, p.TEL, p.DIR, p.SAL'''
